...
export default auth(req => {
	if (!req.auth?.user?.roles?.includes('super-admin') && superAdminUrls.includes(req.nextUrl.pathname)) {
		const baseUrl = new URL(process.env.SITE_LINK ?? 'http://localhost:3000'); //TODO: Add to config file
	...
	if (!req.auth && publisherUrls.includes(req.nextUrl.pathname)) {
		const loginUrl = new URL(`${process.env.SITE_LINK ?? 'http://localhost:3000'}/auth/signIn`);
